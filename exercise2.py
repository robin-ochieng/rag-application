...
def implement_set_api_key(api_key):
    ...
    # Correct env var name is GROQ_API_KEY (exercise expectations)
...
def implement_llama_3_3_model():
    ...
    """
    # Slightly higher temperature for more creative responses (per instructions)
...
def implement_query_model(model, prompt):
    ...
    # Model expects list of messages like LangChain; adapt prompt into that format
    # Our mock currently accepts list and we pass list to stay consistent with spec
    response = model.invoke([("human", prompt.strip())])
    # Mock returns MockAIMessage
...
def implement_compare_models(prompt):
    ...
    if not isinstance(prompt, str) or not prompt.strip():
        raise ValueError("prompt must be a non-empty string")
    m1 = implement_llama_4_model()
    m2 = implement_llama_3_3_model()
    r1 = implement_query_model(m1, prompt)
    r2 = implement_query_model(m2, prompt)
    return {
        "llama-4-8b-instant": r1,
        "llama-3.3-70b-versatile": r2
    }
